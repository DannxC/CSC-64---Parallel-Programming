...
void exchangeGhostZones(int* restrict tabulIn, const int tam, const int tamLocal,
                        const int procAntes, const int procDepois) {
  MPI_Status status;

  // Exchange ghost zones with the previous process
  MPI_Sendrecv(
    /* sendbuf */    &tabulIn[ind2d(1, 0)], tam + 2, MPI_INT, procAntes, 1,
    /* recvbuf */    &tabulIn[ind2d(0, 0)], tam + 2, MPI_INT, procAntes, 0,
    MPI_COMM_WORLD, &status
  );

  // Exchange ghost zones with the next process
  MPI_Sendrecv(
    /* sendbuf */    &tabulIn[ind2d(tamLocal, 0)], tam + 2, MPI_INT, procDepois, 0,
    /* recvbuf */    &tabulIn[ind2d(tamLocal + 1, 0)], tam + 2, MPI_INT, procDepois, 1,
    MPI_COMM_WORLD, &status
  );
}
...
void UmaVida(int* restrict tabulIn, int* restrict tabulOut, const int tam,
  ...
  // Update ghost zones
  exchangeGhostZones(tabulIn, tam, tamLocal, procAntes, procDepois);
