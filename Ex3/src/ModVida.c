...
void UmaVida(int* restrict tabulIn, int* restrict tabulOut, const int tam, 
  ...
  // Enviar e receber as linhas para atualizar as ghost zones
  // Envia a primeira linha interna para o processo anterior e recebe a última linha interna do processo anterior
  if (procAntes != MPI_PROC_NULL) {
    MPI_Sendrecv(&tabulIn[ind2d(1,0)], tam+2, MPI_INT, procAntes, 0,
                 &tabulIn[ind2d(0,0)], tam+2, MPI_INT, procAntes, 0,
                 MPI_COMM_WORLD, &status);
  }

  // Envia a última linha interna para o processo posterior e recebe a primeira linha interna do processo posterior
  if (procDepois != MPI_PROC_NULL) {
    MPI_Sendrecv(&tabulIn[ind2d(tamLocal,0)], tam+2, MPI_INT, procDepois, 0,
                 &tabulIn[ind2d(tamLocal+1,0)], tam+2, MPI_INT, procDepois, 0,
                 MPI_COMM_WORLD, &status);
  }
